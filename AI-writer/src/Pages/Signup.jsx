...
function Signup() {
    ...
    const [selectedPreviousPlan, setselectedPreviousPlan] = useState('')
    ...
    const [NewPlanPurchased, setNewPlanPurchased] = useState(false)
    ...
    const { PlanName, PlanAmount } = useSelector(state => state.Assets);
    ...
    const HandleArticleWordsForCustomPlan = (event, newWordsValue) => {
        setIsPurchasedCustomPlanEditable(true)
        setIsPurchasedCustomContentWordsEditable(true)
    ...
    const HandlePlagiarismWordsForCustomPlan = (event, newPlagiarisedValue) => {
        setIsPurchasedCustomPlanEditable(true)
        setIsPurchasedCustomPlagiarismWordsEditable(true)
    ...
    useEffect(() => {
        if (PlanAmount && selectedPlan) {
            const AmountDifference = Math.round(PlanAmount / 100 - PricePlans[selectedPlan].price)
            setAmountDifferenceInPlan(AmountDifference)
        }


    }, [PlanAmount, selectedPlan, setNewPlanPurchased, CustomPrice])
            ...
            <div className="flex items-center justify-center pb-20 font-poppins">
                <div className="flex py-10 mt-10 max-md:flex-col max-md:justify-center max-md:items-center xl:w-11/12 2xl:w-10/12 md:space-x-6 xl:space-x-16 2xl:space-x-28 max-xl:px-10 ">
                    ...
                    <div className="w-full max-md:justify-center max-md:flex-col max-md:items-center max-md:px-10 md:mt-10 md:w-3/12 ">
                        <h3 className="mb-2 text-2xl text-center ">{PlanName === 'Nil' ? 'You are subscribing for:' : 'Your current plan:'}</h3>
                    ...
                    /> */}
                        ...
                        <AccordianComponent
                            ...
                            AmountDifferenceInPlan={AmountDifferenceInPlan}
                            IsPurchasedCustomPlanEditable={IsPurchasedCustomPlanEditable}
                            IsPurchasedCustomContentWordsEditable={IsPurchasedCustomContentWordsEditable}
                            IsPurchasedCustomPlagiarismWordsEditable={IsPurchasedCustomPlagiarismWordsEditable}
                        />

                        {(!ShowPlanLists && PlanName !== 'CUSTOM') && (<div className="px-6 ">
                        ...
                        {(PlanName === 'CUSTOM' && IsCustomPlanSelected) && (<div className="px-6 ">
                            <button onClick={HandlePurchasedCustomPlan} className="w-full py-2 mt-5 font-semibold text-white rounded-md bg-custom-dark-orange">CHOOSE ANOTHER PLAN</button>
                        </div>)}
                        ...
                        {(IsCustomPlanSelected && PlanName !== 'CUSTOM') && (<div className="px-6 ">
