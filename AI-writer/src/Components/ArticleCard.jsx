...
function ArticleCard({ title, description, buttonText, LinkTo,FunctionToCall, buttonAction, footer }) {
    
    const dispatch = useDispatch()

    const handleClick = () => {
        if (FunctionToCall) {
            FunctionToCall(dispatch);
        }
    };
    ...
    return (
        <div className="w-full p-5 space-y-3 border rounded-md shadow-xl cursor-pointer xl:p-8 xl:space-y-6 border-custom-dark-orange border-opacity-40">
            ...
            <div className="">
                <Link to={LinkTo} onClick={handleClick} className="px-10 py-1 text-center text-white rounded-md max-xl:text-sm xl:py-2 xl:px-16 hover:bg-hover-button-color bg-custom-dark-orange">{buttonText}</Link>
            </div>
